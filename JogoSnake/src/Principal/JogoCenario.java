...
public class JogoCenario extends CenarioPadrao {
    ...
    //Definindo o metodo e fluxo do estado do jogo
	enum Estado {
	...
	private static final int RASTRO_INICIAL = 5;//o tamanho inicial da cobra
		...
		for (int linha = 0; linha < nivelSelecionado.length; linha++) {
			for (int coluna = 1; coluna < nivelSelecionado[0].length; coluna++) {
				if (nivelSelecionado[linha][coluna] != ' ') {
					...
					e.setAtivo(true);
					e.setCor(Color.ORANGE);//cor do obstaculo
	...
	public void atualizar() {
		...
		if (!moveu) {//Direcção da cobra fazendo 0 sem direcção, -1 esquerda e cima, 1 direita e baixo
			if (dy != 0) {
                ...
                //movimentação da da cabeça da cobra guardando os valores em x e y, e salvando os valores de x e y em tx e ty, evitando erros
		if (temporizador >= 20) {
			temporizador = 0;
			moveu = false;
			...
			serpente.setPy(serpente.getPy() + _LARG * dy);
                        ...
                        //verificando se a cabeça da cobra saiu da tela
			...
			if (Util.colide(fruta, serpente)) {
				// Adiciona uma pausa caso a cobra colide com a fruta e continuar com o jogo
				temporizador = -10;
				contadorRastro++;
	...
	public void desenhar(Graphics2D g) {
		...
		if (estado != Estado.JOGANDO) {
			...
			if (estado == Estado.GANHOU)
				...
				texto.desenha(g, "Perdeu!", 180, 180);
		}

		if (Jogo.pausado)
			Jogo.textoPausa.desenha(g, "PAUSADO", largura / 2 - Jogo.textoPausa.getFonte().getSize(), altura / 2);
	}
